...
export function CalorieDial({ eaten, goal }: CalorieDialProps) {
  ...
  return (
    <View className="overflow-hidden rounded-card bg-ink p-6">
      ...
      <Text className="font-bold text-xs uppercase tracking-[3px] text-dial-muted">Today&apos;s fuel</Text>
      <View className="mt-8 flex-row items-end gap-2">
        <Text className="font-display text-6xl text-paper">{eaten}</Text>
        <Text className="mb-2 font-medium text-base text-dial-muted">of {goal} kcal</Text>
      </View>
      <Text className={`mt-3 font-bold text-sm ${over ? "text-citrus" : "text-dial-soft"}`}>{dialCaption(geometry)}</Text>
      <View className="mt-6 h-2 overflow-hidden rounded-full bg-dial-track">
